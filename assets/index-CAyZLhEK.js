...
Wenn du fortfährst, werden alle alten Tagesdaten gelöscht und durch die neuen ersetzt. (Profil-Einstellungen wie Theme und Mail-Vorlagen bleiben erhalten.)`,{title:"Anderer Azubi erkannt",confirmLabel:"Daten ersetzen",variant:"danger"})){s.style.pointerEvents="",s.style.opacity="",a.innerHTML=`
    ...
    <form id="profil-form" class="grid cols-2" style="gap: 24px;">
      ...
      <div class="card">
        ...
        <div class="form-group">
          ...
          <input type="text" name="zeitwirtschaftAnrede" value="${bn(t.zeitwirtschaftAnrede)}" placeholder="z. B. Frau Müller" />
        </div>
      </div>
  ...
  `,c.length&&(requestAnimationFrame(()=>{const k=e.querySelector("#chart-kalender-woche");k&&Wt(k,c)}),It(e.querySelector("#tage-liste-kalender")))}function ur({aktiverMonatKey:e,monatLabel:t,prevMonatKey:n,nextMonatKey:a,tageAnzahl:s,warnungenAnzahl:i}){const l=n?Tt(n):"",d=a?Tt(a):"";return`
    ...
    <div class="grid cols-3" style="margin-bottom: 24px;">
      ...
      </a>`}).join(""):'<p style="color: var(--fg-muted);">Keine Wochen.</p>'}function ia(e,t,n){const a=e.schweregrad==="error"?"!":e.schweregrad==="warn"?"⚠":"i";let s=null;if(e.mailVorlageKey&&t){const i=Xe(e.mailVorlageKey,n==null?void 0:n[e.mailVorlageKey]);if(i){const l=Vt(t,e);s=Kt(t,i,l)}}return`
    <div class="alert severity-${e.schweregrad}">
      <div class="alert-icon">${a}</div>
      <div class="alert-body">
  ...
  `:'<p style="color: var(--fg-muted);">Keine Tage in dieser Auswahl.</p>'}function Wr(e){return{MO:"Montag",DI:"Dienstag",MI:"Mittwoch",DO:"Donnerstag",FR:"Freitag",SA:"Samstag",SO:"Sonntag"}[e]??e}function Nr(e){const[t,n]=e.split("-");return L(new Date(parseInt(t),parseInt(n)-1,1),"LLLL yyyy",{locale:ne})}function Vr(e){const t=String(e).toLowerCase();return t.includes("stempel")?"stempelzeit":t.includes("berufs")?"berufsschule":t.includes("urlaub")?"urlaub":t.includes("feiertag")?"feiertag":t.includes("dienst")?"dienstreise":t.includes("gleit")?"gleitzeit":t.includes("arbeitsunf")?"arbeitsunfaehigkeit":"feiertag"}function ga(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function xn(e){return ga(e)}async function Kr({root:e}){const[t,n,a,s,i]=await Promise.all([ue(),oe(),gt(),ht(),Le()]),l=Object.fromEntries(s.map(r=>[r.key,r])),d=mt(t,n,new Date,void 0,{isAzubi:i}).filter(r=>r.mailVorlageKey),c=Un(a);e.innerHTML=`
    ...
    <div class="card">
      ...
      <div id="vorlagen-editor">
        ${Object.keys(je).map(r=>{const o=Xe(r,l[r]);return Hr(o)}).join("")}
      </div>
    </div>
  `,Rr(e),qr(e)}function Ir(e,t,n){const a=Xe(e.mailVorlageKey,n[e.mailVorlageKey]);if(!a)return"";const s=Vt(t,e),i=Jn(a,s),l=Kt(t,a,s);return`
...
${a}`;await navigator.clipboard.writeText(s),O("Mail-Text kopiert.")})})}function te(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function Lt(e){return te(e)}async function jr({root:e}){const[t,n]=await Promise.all([oe(),ht()]),a=Object.fromEntries(n.map(s=>[s.key,s]));e.innerHTML=`
    ...
    <div class="grid cols-2">
      ...
      <div class="card" style="grid-column: 1 / -1;">
        ...
        <div id="vorlagen-editor">
          ${Object.keys(je).map(s=>{const i=Xe(s,a[s]);return Zr(i)}).join("")}
        </div>
      </div>
  ...
  `,e.querySelector("#pause-sel").addEventListener("change",async s=>{const i={...t,pausenRegel:s.target.value};await lt(i),O("Pausen-Regel gespeichert.")}),e.querySelectorAll(".btn-save-vorlage").forEach(s=>{s.addEventListener("click",async()=>{const i=s.getAttribute("data-key"),l=s.closest(".vorlagen-item"),d=l.querySelector(".v-betreff").value,c=l.querySelector(".v-text").value;await ct(i,{betreff:d,text:c}),O("Vorlage gespeichert.")})}),e.querySelectorAll(".btn-reset-vorlage").forEach(s=>{s.addEventListener("click",async()=>{const i=s.getAttribute("data-key"),l=je[i];l&&await le(`Deine Anpassungen an "${l.titel}" gehen verloren.`,{title:"Vorlage zurücksetzen?",confirmLabel:"Zurücksetzen",variant:"danger"})&&(await ct(i,{betreff:l.betreff,text:l.text}),O("Zurückgesetzt."),setTimeout(()=>location.reload(),400))})})}function Zr(e){return`
    <div class="vorlagen-item" data-key="${e.key}" style="border: 1px solid var(--border); border-radius: var(--radius-md); padding: 16px; margin-bottom: 16px;">
      <div class="card-split" style="margin-bottom: 12px;">
        <div>
