...
`):"",h=d?t[0].schweregrad:null;return`
  ...
  `,At(h,e,d,v);const B=()=>e.querySelectorAll("#zeitraum-picker button, #zeitraum-picker-2 button");B().forEach(S=>{S.addEventListener("click",async()=>{if(S.disabled)return;const P=S.getAttribute("data-key"),I=P==="alles"?"alles":parseInt(P,10);B().forEach(M=>M.classList.toggle("active",M.getAttribute("data-key")===P)),await rs("dashboardZeitraum",I),At(I,e,d,v)})}),e.addEventListener("click",async S=>{const P=S.target.closest(".ist-part");if(P){S.preventDefault(),S.stopPropagation();const z=P.getAttribute("data-val");try{await navigator.clipboard.writeText(z),P._copyResetTimer&&clearTimeout(P._copyResetTimer),P.classList.add("ok"),P.textContent="✓",P._copyResetTimer=setTimeout(()=>{P.classList.remove("ok"),P.textContent=z,P._copyResetTimer=null},800)}catch{L(`Bitte kopieren: ${z}`)}return}const I=S.target.closest(".btn-dismiss");if(!I)return;const M=I.getAttribute("data-datum"),K=I.getAttribute("data-typ");if(!M||!K)return;await fs(M,K),L("Warnung ausgeblendet.");const k=I.closest(".alert");k&&(k.style.display="none")})}function At(e,t,n,a=[]){const s=Da(n,e),i=Ea(s);requestAnimationFrame(()=>{const d=t.querySelector("#saldo-chart");gt(d,s)});const l=t.querySelector("#hero-bewegung");if(l){const d=i>.05?"up":i<-.05?"down":"flat",o=i>.05?"↑":i<-.05?"↓":"→";l.className=`saldo-trend ${d}`,l.innerHTML=`${o} ${Math.abs(i)<.01?"Unverändert":se(i,{signed:!0})} im Zeitraum`}const c=t.querySelector("#tage-info");if(c){const d=s.length;c.textContent=`${d} ${d===1?"Tag":"Tage"}`}const r=t.querySelector("#tage-liste");r&&(r.innerHTML=Ue(s.slice().reverse(),{warnungen:a.filter(d=>s.some(o=>o.datum===d.datum))}))}function Ts(e,t,n){const a=e.schweregrad==="error"?"!":e.schweregrad==="warn"?"⚠":"i";let s=null;if(e.mailVorlageKey&&t){const i=Pe(e.mailVorlageKey,n==null?void 0:n[e.mailVorlageKey]);if(i){const l=ft(t,e);s=mt(t,i,l)}}return`
...
Wenn du fortfährst, werden alle alten Tagesdaten gelöscht und durch die neuen ersetzt. (Profil-Einstellungen wie Theme und Mail-Vorlagen bleiben erhalten.)`,{title:"Anderer Azubi erkannt",confirmLabel:"Daten ersetzen",variant:"danger"})){s.style.pointerEvents="",s.style.opacity="",a.innerHTML=`
      ...
      <div class="card" style="grid-column: 1 / -1;">
        ...
        ${a?`
          ...
          <div class="tag-detail">
            ...
            <div class="tag-detail-value"><strong>${((c=n.wochentage14Taegig)==null?void 0:c.map(d=>Bt[d]).join(", "))||"—"}</strong>${n.referenzDatum14Taegig?` <span class="hint">(Anker: ${F(new Date(n.referenzDatum14Taegig))})</span>`:""}</div>
          </div>
          <div class="tag-detail">
  ...
  `,e.querySelector("#profil-form").addEventListener("submit",async d=>{var h,g;d.preventDefault();const o=new FormData(d.target),u={...t,ueber18:o.get("ueber18")==="on",zeitwirtschaftEmail:((h=o.get("zeitwirtschaftEmail"))==null?void 0:h.trim())||"Zeitwirtschaft.Aichtal@putzmeister.com",zeitwirtschaftAnrede:((g=o.get("zeitwirtschaftAnrede"))==null?void 0:g.trim())||"Frau Kanarya"};await Ke(u),L("Profil gespeichert.")})}function Wt(e){return String(e??"").replace(/"/g,"&quot;")}async function di({params:e,root:t}){var d,o,u,h;const n=e.datum,a=await p.tage.get(n),s=await J();if(!a){t.innerHTML=`<h1 class="view-title">Tag nicht gefunden</h1><p>Für ${n} liegen keine Daten vor.</p><a href="#/" class="btn">← Dashboard</a>`;return}const i=_(n),l=a.tagestyp==="Stempelzeit"?Jt(a,s):null,c=qe(i,s.berufsschulMuster),r=Ut(a.zeitSpannen);t.innerHTML=`
  ...
  `,r.length&&(requestAnimationFrame(()=>{const w=e.querySelector("#chart-kalender-woche");w&&gt(w,r)}),pt(e.querySelector("#tage-liste-kalender")))}function vi({aktiverMonatKey:e,monatLabel:t,prevMonatKey:n,nextMonatKey:a,tageAnzahl:s,warnungenAnzahl:i}){const l=n?it(n):"",c=a?it(a):"";return`
    ...
    </div>`}function wi(e,t){var s;const n={};for(const i of e)n[s=i.wocheKey]??(n[s]={key:i.wocheKey,tage:[],ist:0,soll:0}),n[i.wocheKey].tage.push(i),n[i.wocheKey].ist+=He(i),n[i.wocheKey].soll+=ct(i);const a=Object.keys(n).sort();return a.length?a.map(i=>{var h;const l=n[i],c=+(l.ist-l.soll).toFixed(2),r=c>.05?"var(--success)":c<-.05?"var(--danger)":"var(--fg-muted)",d=(h=l.tage.slice().sort((g,m)=>g.datum.localeCompare(m.datum))[0])==null?void 0:h.datum,o=d?Ae(_(d)):"",u=i.split("-")[1]||"?";return`
...
Vielen Dank!`,l=Vt(s,a),c=Vt(i,a),r=new URLSearchParams;return r.set("cc","anika.kailer@putzmeister.com"),r.set("subject",l),r.set("body",c),`mailto:annika.mayer@putzmeister.com?${r.toString().replace(/\+/g,"%20")}`}function Vt(e,t){return e.replace(/\{(\w+)\}/g,(n,a)=>t[a]!=null?String(t[a]):`{${a}}`)}function Di(e){const t=new Set;for(const n of e)n.tagestyp==="Berufsschule"&&(n.ist??0)>0&&t.add(n.monatKey);return[...t].sort().reverse()}function Ai(e,t,n,a){var h,g;const s=t==null?void 0:t.berufsschulMuster;if(!s||!((h=s.wochentageWoechentlich)!=null&&h.length)&&!((g=s.wochentage14Taegig)!=null&&g.length))return{ausstehend:[],imMonatGesamt:n.length,vorhandenAnzahl:n.length};const[i,l]=e.split("-").map(m=>parseInt(m,10)),c=new Date(i,l-1,1),r=new Date(i,l,0),d=new Set(n.map(m=>m.datum)),o=[];let u=new Date(c);for(;u<=r;){if(!De(u)&&qe(u,s)){const m=E(u,"yyyy-MM-dd");u>a&&!d.has(m)&&o.push(m)}u=ze(u,1)}return{ausstehend:o,imMonatGesamt:n.length+o.length,vorhandenAnzahl:n.length}}function Ei(e){const t=e instanceof ArrayBuffer?e:e!=null&&e.buffer?e.buffer.slice(e.byteOffset,e.byteOffset+e.byteLength):null;if(!t||t.byteLength<24)return null;const n=new DataView(t);if(n.getUint32(0)===2303741511&&n.getUint32(4)===218765834)return{width:n.getUint32(16),height:n.getUint32(20)};if(n.getUint16(0)===65496){let a=2;for(;a+9<n.byteLength;){if(n.getUint8(a)!==255)return null;const s=n.getUint8(a+1);if(s>=192&&s<=207&&s!==196&&s!==200&&s!==204)return{height:n.getUint16(a+5),width:n.getUint16(a+7)};const l=n.getUint16(a+2);if(l<2)return null;a+=2+l}}return null}function Pi(e,t){const n=URL.createObjectURL(e),a=document.createElement("a");a.href=n,a.download=t,document.body.appendChild(a),a.click(),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(n),1e3)}async function Me({root:e}){var v,w,y,B,S,P,I,M,K;const t=await xe();if(!t){Ti(e);return}const[n,a,s,i]=await Promise.all([te(),J(),Ee(),hs("fahrgeld_abgabe")]),l=s!=null&&s.stichtagAuswertung?new Date(s.stichtagAuswertung):new Date,c=Di(n),r=new Date,d=`${r.getFullYear()}-${String(r.getMonth()+1).padStart(2,"0")}`;c.includes(d)||c.unshift(d);const o=c;e.innerHTML=`
    ...
    `:(()=>{var C;const k=o.map(($,W)=>{const H=Ot(n,$),ne=+(H.length*(t.konstanten.betragProTag??0)).toFixed(2),X=H.length>10,R=Ai($,a,H,l),Y=R.ausstehend.length,Te=+((H.length+Y)*(t.konstanten.betragProTag??0)).toFixed(2),Ge=Y>0?R.ausstehend.map(Je=>F(_(Je))).join(", "):"";return{mk:$,bs:H,summe:ne,ueberzaehlig:X,offen:Y,offenListe:Ge,summeMitOffen:Te,idx:W}}),z=((C=k.find($=>$.bs.length>0))==null?void 0:C.mk)??k[0].mk,D=k.find($=>$.mk===z)??k[0];return`
          ...
          <div class="card">
            ...
            <div class="fahrtgeld-monate">
              ${k.map($=>{const W=$.mk===z?"checked":"",H=`data-monat="${x($.mk)}" data-offen="${$.offen}" data-offen-liste="${x($.offenListe)}" data-anzahl="${$.bs.length}" data-summe="${$.summe.toFixed(2).replace(".",",")}" data-summe-mit-offen="${$.summeMitOffen.toFixed(2).replace(".",",")}" data-monat-label="${x(Be($.mk))}" data-ueberzaehlig="${$.ueberzaehlig?"1":"0"}"`;return`
                  <label class="fahrtgeld-monat-row${$.offen>0?" has-pending":""}${$.bs.length===0?" empty":""}" ${H}>
  ...
  `,(B=e.querySelector("#btn-template-replace"))==null||B.addEventListener("click",()=>{e.querySelector("#fahrtgeld-replace-input").click()}),(S=e.querySelector("#fahrtgeld-replace-input"))==null||S.addEventListener("change",async k=>{var D;const z=(D=k.target.files)==null?void 0:D[0];z&&await rt(z,e)}),(P=e.querySelector("#btn-template-clear"))==null||P.addEventListener("click",async()=>{await U("Beim nächsten Öffnen wirst du gebeten, eine neue Vorlage hochzuladen.",{title:"Vorlage löschen?",confirmLabel:"Löschen",variant:"danger"})&&(await cs(),L("Vorlage gelöscht."),Me({root:e}))}),(I=e.querySelector("#btn-signature-upload"))==null||I.addEventListener("click",()=>{e.querySelector("#fahrtgeld-signature-input").click()}),(M=e.querySelector("#fahrtgeld-signature-input"))==null||M.addEventListener("change",async k=>{var D;const z=(D=k.target.files)==null?void 0:D[0];z&&await Fi(z,e)}),(K=e.querySelector("#btn-signature-clear"))==null||K.addEventListener("click",async()=>{if(!await U("Die hochgeladene Unterschrift wird entfernt — generierte Excel haben dann keine Unterschrift mehr.",{title:"Unterschrift entfernen?",confirmLabel:"Entfernen",variant:"danger"}))return;const k=await xe();k&&(await ht({...k,unterschrift:null}),L("Unterschrift entfernt."),Me({root:e}))});const u=e.querySelector("#btn-fahrtgeld-generate"),h=e.querySelector("#btn-fahrtgeld-mail"),g=e.querySelector(".fahrtgeld-generate-monat"),m=e.querySelector("#fahrtgeld-generate-detail"),f=e.querySelectorAll(".fahrtgeld-monat-radio");function b(){const k=e.querySelector(".fahrtgeld-monat-radio:checked");if(!k)return;const z=k.closest(".fahrtgeld-monat-row");if(!z)return;const D=k.value,C=parseInt(z.getAttribute("data-anzahl")||"0",10),$=z.getAttribute("data-summe")||"0,00",W=parseInt(z.getAttribute("data-offen")||"0",10),H=z.getAttribute("data-offen-liste")||"",ne=z.getAttribute("data-summe-mit-offen")||$,X=z.getAttribute("data-monat-label")||"",R=z.getAttribute("data-ueberzaehlig")==="1";if(g&&(g.textContent=X),m){const Y=[`${C} BS-Tag${C===1?"":"e"} erfasst · ${$} €`];W>0&&Y.push(`<br><span class="fahrtgeld-pending">⏳ Laut Schul-Rhythmus kommen noch ${W} BS-Tag${W===1?"":"e"}: ${x(H)} (mit allen: ${ne} €)</span>`),R&&Y.push(`<br><span style="color: var(--warn); font-weight: 600;">· ${C-10} überzählig (max 10/Monat)</span>`),m.innerHTML=Y.join("")}u&&(u.disabled=C===0),h&&(h.href=Kt(D,i),C===0?h.setAttribute("aria-disabled","true"):h.removeAttribute("aria-disabled"))}f.forEach(k=>k.addEventListener("change",b)),u==null||u.addEventListener("click",async()=>{var ne,X;const k=e.querySelector(".fahrtgeld-monat-radio:checked");if(!k){N("Bitte einen Monat auswählen.",{type:"warn"});return}const z=k.closest(".fahrtgeld-monat-row"),D=k.value,C=parseInt((z==null?void 0:z.getAttribute("data-offen"))||"0",10),$=(z==null?void 0:z.getAttribute("data-offen-liste"))||"";if(C>0&&!await U(`Laut deinem Schul-Rhythmus kommen in diesem Monat noch ${C} BS-Tag${C===1?"":"e"}:

${$}

Diese Tage sind noch nicht im Zeitnachweis-PDF und werden in der Excel fehlen. Trotzdem jetzt für die bisher erfassten Tage generieren?`,{title:"Noch ausstehende BS-Tage",confirmLabel:"Trotzdem generieren"}))return;u.disabled=!0;const W=u.querySelector(".btn-generate-label"),H=W==null?void 0:W.textContent;W&&(W.textContent="Erstelle …");try{const R=await xe(),Y=await te(),Te=Ot(Y,D);if(!Te.length){N("Für diesen Monat sind keine BS-Anwesenheits-Tage erkannt.",{type:"warn"});return}const Ge=(ne=R.unterschrift)==null?void 0:ne.bytes,Je=(X=R.unterschrift)==null?void 0:X.extension,{blob:Sn,dateiname:wt,ueberzaehlig:Xe}=await Mi({templateBytes:R.bytes,monatKey:D,bsTage:Te,konstanten:R.konstanten,unterschriftBytes:Ge,unterschriftExtension:Je});Pi(Sn,wt),L(`Excel erstellt: ${wt}${Xe>0?` (${Xe} weiterer Tag${Xe===1?"":"e"} passte/n nicht in die Vorlage)`:""}`)}catch(R){console.error("Fahrtgeld-Generierung fehlgeschlagen:",R),N(`Fehler: ${R.message||R}`,{type:"error",duration:8e3})}finally{u.disabled=!1,W&&H&&(W.textContent=H)}})}function Ti(e){e.innerHTML=`
    <h1 class="view-title">Fahrgelderstattung</h1>
  ...
  `;const t=e.querySelector("#fahrtgeld-dropzone"),n=e.querySelector("#fahrtgeld-upload-input");t==null||t.addEventListener("click",()=>n.click()),t==null||t.addEventListener("keydown",a=>{(a.key==="Enter"||a.key===" ")&&(a.preventDefault(),n.click())}),n==null||n.addEventListener("change",async a=>{var i;const s=(i=a.target.files)==null?void 0:i[0];s&&await rt(s,e)}),["dragenter","dragover"].forEach(a=>{t==null||t.addEventListener(a,s=>{s.preventDefault(),s.stopPropagation(),t.classList.add("is-dragover")})}),["dragleave","drop"].forEach(a=>{t==null||t.addEventListener(a,s=>{s.preventDefault(),s.stopPropagation(),t.classList.remove("is-dragover")})}),t==null||t.addEventListener("drop",async a=>{var i,l;const s=(l=(i=a.dataTransfer)==null?void 0:i.files)==null?void 0:l[0];s&&await rt(s,e)})}async function Fi(e,t){try{if(!["image/png","image/jpeg"].includes(e.type)&&!/\.(png|jpe?g)$/i.test(e.name)){N("Bitte ein PNG oder JPG hochladen.",{type:"warn"});return}if(e.size>2*1024*1024){N(`Die Bilddatei ist zu groß (${(e.size/1024/1024).toFixed(1)} MB). Maximal 2 MB — bitte komprimiert oder kleiner hochladen.`,{type:"warn",duration:8e3});return}const a=await e.arrayBuffer(),s=/\.png$/i.test(e.name)||e.type==="image/png"?"png":"jpeg",i=await new Promise((c,r)=>{const d=new FileReader;d.onload=()=>c(d.result),d.onerror=r,d.readAsDataURL(e)}),l=await xe();if(!l){N("Bitte zuerst eine Excel-Vorlage hochladen.",{type:"warn"});return}await ht({...l,unterschrift:{bytes:a,extension:s,dataUrl:i,hochgeladenAm:new Date().toISOString()}}),L("Unterschrift gespeichert."),Me({root:t})}catch(n){console.error("Unterschrift-Upload fehlgeschlagen:",n),N(`Fehler beim Verarbeiten: ${n.message||n}`,{type:"error",duration:8e3})}}async function rt(e,t){try{const n=await e.arrayBuffer(),a=await Si(n);if(!a.ok){N(`Vorlage nicht erkannt: ${a.fehler}`,{type:"error",duration:8e3});return}const s=await xe();let i=s==null?void 0:s.unterschrift;if(!i&&a.unterschriftAuto){const l=Li(a.unterschriftAuto.bytes,a.unterschriftAuto.extension);i={bytes:a.unterschriftAuto.bytes,extension:a.unterschriftAuto.extension,dataUrl:l,quelle:"auto-aus-vorlage",hochgeladenAm:new Date().toISOString()}}await ht({bytes:n,konstanten:a.konstanten,unterschrift:i,hochgeladenAm:new Date().toISOString(),dateiname:e.name}),a.unterschriftAuto&&!(s!=null&&s.unterschrift)?L(`Vorlage gespeichert: ${a.konstanten.name||e.name} (Unterschrift aus Vorlage übernommen)`):L(`Vorlage gespeichert: ${a.konstanten.name||e.name}`),document.dispatchEvent(new CustomEvent("better-ess:fahrgeld-template-uploaded")),Me({root:t})}catch(n){console.error("Vorlage-Upload fehlgeschlagen:",n),N(`Fehler beim Verarbeiten: ${n.message||n}`,{type:"error",duration:8e3})}}function Li(e,t){const n=new Uint8Array(e);let a="";for(let l=0;l<n.length;l++)a+=String.fromCharCode(n[l]);const s=btoa(a);return`data:${t==="png"?"image/png":"image/jpeg"};base64,${s}`}async function _i({root:e}){const t=await te();if(!t.length){e.innerHTML='<h1 class="view-title">Berichtsheft</h1><p class="view-subtitle">Keine Daten.</p><a href="#/daten" class="btn primary">Zum Upload</a>';return}const n=[...new Set(t.map(i=>i.monatKey))].sort().reverse();e.innerHTML=`
...
`);navigator.clipboard.writeText(l).then(()=>L(`${i.length} Zeilen kopiert.`))});function s(){const i=a.value,c=(i?t.filter(r=>r.monatKey===i):t).filter(r=>r.ist!=null&&r.ist>0&&r.tagestyp!=="Wochenende");e.querySelector("#liste").innerHTML=Ci(c),e.querySelectorAll(".copy-val").forEach(r=>{r.addEventListener("click",async()=>{const d=r.getAttribute("data-val");await navigator.clipboard.writeText(d),r.classList.add("ok");const o=r.textContent;r.textContent="✓ kopiert",setTimeout(()=>{r.classList.remove("ok"),r.textContent=o},1200)})})}s()}function Ci(e){return e.length?`
    <table class="table">
      <thead>
        <tr>
          <th>Datum</th>
      ...
      <tbody>
        ${e.map(t=>{const n=Oe(t.ist),a=`${F(_(t.datum))}	${n}`;return`
            <tr data-copy="${It(a)}">
              <td class="tabular">${F(_(t.datum))}</td>
  ...
  `,Ri(e),Hi(e)}function Vi(e,t,n){const a=Pe(e.mailVorlageKey,n[e.mailVorlageKey]);if(!a)return"";const s=ft(t,e),i=hn(a,s),l=mt(t,a,s);return`
...
${q(i.text)}
  ...
  `}function Ri(e){e.querySelectorAll(".btn-save-vorlage").forEach(t=>{t.addEventListener("click",async()=>{const n=t.getAttribute("data-key"),a=t.closest(".vorlagen-item"),s=a.querySelector(".v-betreff").value,i=a.querySelector(".v-text").value;await Ve(n,{betreff:s,text:i}),L("Vorlage gespeichert.")})}),e.querySelectorAll(".btn-reset-vorlage").forEach(t=>{t.addEventListener("click",async()=>{const n=t.getAttribute("data-key"),a=Se[n];a&&await U(`Deine Anpassungen an "${a.titel}" gehen verloren.`,{title:"Vorlage zurücksetzen?",confirmLabel:"Zurücksetzen",variant:"danger"})&&(await Ve(n,{betreff:a.betreff,text:a.text}),L("Zurückgesetzt."),setTimeout(()=>location.reload(),400))})})}function Hi(e){e.querySelectorAll(".btn-copy-mail").forEach(t=>{t.addEventListener("click",async()=>{const n=t.getAttribute("data-subject"),a=t.getAttribute("data-body"),s=`Betreff: ${n}

${a}`;await navigator.clipboard.writeText(s),L("Mail-Text kopiert.")})})}function q(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function lt(e){return q(e)}async function Ni({root:e}){const[t,n]=await Promise.all([J(),je()]),a=Object.fromEntries(n.map(s=>[s.key,s]));e.innerHTML=`
  ...
  `,e.querySelector("#pause-sel").addEventListener("change",async s=>{const i={...t,pausenRegel:s.target.value};await Ke(i),L("Pausen-Regel gespeichert.")}),e.querySelectorAll(".btn-save-vorlage").forEach(s=>{s.addEventListener("click",async()=>{const i=s.getAttribute("data-key"),l=s.closest(".vorlagen-item"),c=l.querySelector(".v-betreff").value,r=l.querySelector(".v-text").value;await Ve(i,{betreff:c,text:r}),L("Vorlage gespeichert.")})}),e.querySelectorAll(".btn-reset-vorlage").forEach(s=>{s.addEventListener("click",async()=>{const i=s.getAttribute("data-key"),l=Se[i];l&&await U(`Deine Anpassungen an "${l.titel}" gehen verloren.`,{title:"Vorlage zurücksetzen?",confirmLabel:"Zurücksetzen",variant:"danger"})&&(await Ve(i,{betreff:l.betreff,text:l.text}),L("Zurückgesetzt."),setTimeout(()=>location.reload(),400))})})}function ji(e){return`
    <div class="vorlagen-item" data-key="${e.key}" style="border: 1px solid var(--border); border-radius: var(--radius-md); padding: 16px; margin-bottom: 16px;">
      <div class="card-split" style="margin-bottom: 12px;">
        <div>
  ...
  `}function We(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function qi(e){return We(e)}async function Rt(){document.documentElement.setAttribute("data-css-ready",""),Gn();const e=document.getElementById("content");await ss()&&setTimeout(()=>{N('Parser wurde aktualisiert. Bitte lade deinen Zeitnachweis unter "Daten" neu hoch.',{type:"info",duration:12e3})},500),await as(),await ts(),await ns(),V("/",Dt),V("/dashboard",Dt),V("/analytics",fi),V("/kalender",et),V("/kalender/typ/:typ",tt),V("/kalender/wochentag/:wd",tt),V("/kalender/pause/:bucket",tt),V("/kalender/:yyyymm",et),V("/kalender/:yyyymm/:yyyykw",et),V("/tag/:datum",di),V("/berichtsheft",_i),V("/mails",Ki),V("/fahrtgeld",Me),V("/profil",ci),V("/daten",ni),V("/einstellungen",Ni),Ja(e),location.hash||Ga("/");try{const t=await p.tage.count();Gs(t>0)&&setTimeout(()=>mn(),700)}catch(t){console.warn("[boot] Tour-Auto-Start-Check fehlgeschlagen:",t)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Rt):Rt();
