...
`):"",g=o?t[0].schweregrad:null;return`
  ...
  `}function es(e){return Yn[e]??e}function ts(e){const t={error:3,warn:2,info:1};let a=e[0];for(const n of e)(t[n.schweregrad]||0)>(t[a.schweregrad]||0)&&(a=n);return a.schweregrad==="error"?"!":a.schweregrad==="warn"?"⚠":"i"}function as(e){const t=String(e).toLowerCase();return t.includes("stempel")?"stempelzeit":t.includes("berufs")?"berufsschule":t.includes("urlaub")?"urlaub":t.includes("feiertag")?"feiertag":t.includes("dienst")?"dienstreise":t.includes("gleit")?"gleitzeit":t.includes("arbeitsunf")?"arbeitsunfaehigkeit":"feiertag"}function st(e){e._istCopyAttached||(e._istCopyAttached=!0,e.addEventListener("click",async t=>{const a=t.target.closest(".ist-part");if(!a)return;t.preventDefault(),t.stopPropagation();const n=a.getAttribute("data-val");try{await navigator.clipboard.writeText(n);const s=a.textContent;a.classList.add("ok"),a.textContent="✓",setTimeout(()=>{a.classList.remove("ok"),a.textContent=s},800)}catch{E(`Bitte kopieren: ${n}`)}}))}const ft=[{key:7,label:"1 Woche"},{key:14,label:"2 Wochen"},{key:30,label:"1 Monat"},{key:90,label:"3 Monate"},{key:"alles",label:"Alles"}];async function pt({root:e}){const[t,a,n,s,i,l,c]=await Promise.all([I(),ie(),V(),Ya(),Ha("dashboardZeitraum",14),nt(),ce()]),r=Object.fromEntries(l.map(y=>[y.key,y]));if(n.length===0){e.innerHTML=ss();return}const o=Be(n,a),d=o.length;let u=i;typeof u=="number"&&u>d&&(u="alles");const g=new Set(s.map(y=>y.key)),h=c!=null&&c.stichtagAuswertung?new Date(c.stichtagAuswertung):new Date,f=Se(n,t,h,g),p=Ht(a,g),b=[...f,...p],m=Cn(o),$=m>.05?"positive":m<-.05?"negative":"";e.innerHTML=`
    ...
    <div class="saldo-hero" style="margin-bottom: 32px; align-items: stretch;">
      <div class="saldo-hero-text" style="min-width: 260px;">
        ...
        <div class="saldo-value ${$}">${C(m,{signed:!0,suffix:""})}</div>
  ...
  `,mt(u,e,o,b);const k=()=>e.querySelectorAll("#zeitraum-picker button, #zeitraum-picker-2 button");k().forEach(y=>{y.addEventListener("click",async()=>{if(y.disabled)return;const S=y.getAttribute("data-key"),x=S==="alles"?"alles":parseInt(S,10);k().forEach(K=>K.classList.toggle("active",K.getAttribute("data-key")===S)),await ja("dashboardZeitraum",x),mt(x,e,o,b)})}),e.addEventListener("click",async y=>{const S=y.target.closest(".ist-part");if(S){y.preventDefault(),y.stopPropagation();const F=S.getAttribute("data-val");try{await navigator.clipboard.writeText(F);const G=S.textContent;S.classList.add("ok"),S.textContent="✓",setTimeout(()=>{S.classList.remove("ok"),S.textContent=G},800)}catch{E(`Bitte kopieren: ${F}`)}return}const x=y.target.closest(".btn-dismiss");if(!x)return;const K=x.getAttribute("data-datum"),T=x.getAttribute("data-typ");if(!K||!T)return;await Ja(K,T),E("Warnung ausgeblendet.");const H=x.closest(".alert");H&&(H.style.display="none")})}function mt(e,t,a,n=[]){const s=Bn(a,e),i=Vn(s);requestAnimationFrame(()=>{const o=t.querySelector("#saldo-chart");Ve(o,s)});const l=t.querySelector("#hero-bewegung");if(l){const o=i>.05?"up":i<-.05?"down":"flat",d=i>.05?"↑":i<-.05?"↓":"→";l.className=`saldo-trend ${o}`,l.innerHTML=`${d} ${Math.abs(i)<.01?"Unverändert":C(i,{signed:!0})} im Zeitraum`}const c=t.querySelector("#tage-info");if(c){const o=s.length;c.textContent=`${o} ${o===1?"Tag":"Tage"}`}const r=t.querySelector("#tage-liste");r&&(r.innerHTML=_e(s.slice().reverse(),{warnungen:n.filter(o=>s.some(d=>d.datum===o.datum))}))}function ns(e,t,a){const n=e.schweregrad==="error"?"!":e.schweregrad==="warn"?"⚠":"i";let s=null;if(e.mailVorlageKey&&t){const i=Ce(e.mailVorlageKey,a==null?void 0:a[e.mailVorlageKey]);if(i){const l=qt(t,e);s=Zt(t,i,l)}}return`
...
Fortfahren?`)){s.style.pointerEvents="",s.style.opacity="",n.innerHTML=`
      ...
      <div class="card" style="grid-column: 1 / -1;">
        ...
        ${n?`
          ...
          <div class="tag-detail">
            ...
            <div class="tag-detail-value"><strong>${((c=a.wochentage14Taegig)==null?void 0:c.map(o=>$t[o]).join(", "))||"—"}</strong>${a.referenzDatum14Taegig?` <span class="hint">(Anker: ${L(new Date(a.referenzDatum14Taegig))})</span>`:""}</div>
          </div>
          <div class="tag-detail">
  ...
  `,e.querySelector("#profil-form").addEventListener("submit",async o=>{var g,h;o.preventDefault();const d=new FormData(o.target),u={...t,ueber18:d.get("ueber18")==="on",zeitwirtschaftEmail:((g=d.get("zeitwirtschaftEmail"))==null?void 0:g.trim())||"Zeitwirtschaft.Aichtal@putzmeister.com",zeitwirtschaftAnrede:((h=d.get("zeitwirtschaftAnrede"))==null?void 0:h.trim())||"Frau Kanarya"};await Pe(u),E("Profil gespeichert.")})}function St(e){return String(e??"").replace(/"/g,"&quot;")}async function Fs({root:e}){var s;const t=await ie(),a=await V(),n={};for(const i of a)n[s=i.monatKey]??(n[s]=[]),n[i.monatKey].push(i);if(t.length===0){e.innerHTML=Vs("Noch keine Monatsdaten. Lade zuerst ein PDF unter Daten hoch.");return}e.innerHTML=`
          ...
          <a href="#/monat/${i.monatKey}" class="card" style="text-decoration: none; color: inherit; display: block;">
            <div class="card-split" style="margin-bottom: 12px;">
              ...
              <span class="saldo-trend ${u>.05?"up":u<-.05?"down":"flat"}" style="margin: 0; font-size: 12px;">
                Saldo: <strong>${C(u??0,{signed:!0})}</strong>
              </span>
            </div>
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 12px;">
              ...
              <div>
                ...
                <div class="tabular" style="font-size: 14px; font-weight: 500; color: ${d>.05?"var(--success)":d<-.05?"var(--danger)":"var(--fg)"};">${C(d,{signed:!0})}</div>
              </div>
            </div>
          </a>
        `}).join("")}
  ...
  `,requestAnimationFrame(()=>{Ve(t.querySelector("#chart-monat"),o)}),st(t.querySelector("#tage-liste-monat"))}function Ws(e){var i,l,c,r;const t=((i=e.glz)==null?void 0:i.saldoGesamt)??0,a=((l=e.glz)==null?void 0:l.saldoVorperiode)??0,n=((c=e.glz)==null?void 0:c.saldoAktuellePeriode)??0,s=o=>o>.05?"var(--success)":o<-.05?"var(--danger)":"var(--fg)";return`
    <div class="grid cols-4" style="margin-bottom: 24px;">
      <div class="card">
        ...
        <div class="tabular" style="font-size: 28px; font-weight: 650; color: ${s(t)};">${C(t,{signed:!0})}</div>
      </div>
      <div class="card">
        <h3>Vorperiode</h3>
        <div class="tabular" style="font-size: 28px; font-weight: 650; color: ${s(a)};">${C(a,{signed:!0})}</div>
      </div>
      <div class="card">
        <h3>Akt. Periode</h3>
        <div class="tabular" style="font-size: 28px; font-weight: 650; color: ${s(n)};">${C(n,{signed:!0})}</div>
      </div>
      <div class="card">
        <h3>Produktivstd.</h3>
        <div class="tabular" style="font-size: 28px; font-weight: 650;">${C(((r=e.zeitkonten)==null?void 0:r.produktivstunden)??0,{suffix:""})}</div>
      </div>
    </div>
  `}function Bs(e){var n;const t={};for(const s of e)t[n=s.wocheKey]??(t[n]={key:s.wocheKey,tage:[],ist:0,soll:0}),t[s.wocheKey].tage.push(s),t[s.wocheKey].ist+=de(s),t[s.wocheKey].soll+=Me(s);const a=Object.keys(t).sort();return a.length?a.map(s=>{var u;const i=t[s],l=+(i.ist-i.soll).toFixed(2),c=l>.05?"var(--success)":l<-.05?"var(--danger)":"var(--fg-muted)",r=(u=i.tage[0])==null?void 0:u.datum,o=r?$e(P(r)):"",d=s.split("-")[1]||"?";return`
      <a href="#/woche/${s}" class="list-item" style="text-decoration: none; color: inherit;">
        ...
        <div class="tabular" style="font-weight: 600; font-size: 16px; color: ${c};">${C(l,{signed:!0})}</div>
  ...
  `}async function Is({params:e,root:t}){const a=e.yyyykw,[n,s,i,l]=await Promise.all([V(),ie(),I(),ce()]),r=Be(n,s).filter(p=>p.wocheKey===a);if(!r.length){t.innerHTML=`<h1 class="view-title">KW ${w(a)}</h1><p>Keine Daten.</p><a href="#/woche" class="btn">← Alle Wochen</a>`;return}const o=r.reduce((p,b)=>p+de(b),0),d=r.reduce((p,b)=>p+Me(b),0),u=+(o-d).toFixed(2),g=p=>p>.05?"var(--success)":p<-.05?"var(--danger)":"var(--fg)",h=l!=null&&l.stichtagAuswertung?new Date(l.stichtagAuswertung):new Date,f=Se(n,i,h).filter(p=>r.some(b=>b.datum===p.datum));t.innerHTML=`
    ...
    <div class="grid cols-3" style="margin-bottom: 24px;">
      ...
      <div class="card">
        <h3>Differenz</h3>
        <div class="tabular" style="font-size: 28px; font-weight: 650; color: ${g(u)};">${C(u,{signed:!0})}</div>
      </div>
    </div>
  ...
  `}function Qt(e){const[t,a]=e.split("-");return`KW ${a} · ${t}`}async function Hs({params:e,root:t}){var o,d,u;const a=e.datum,n=await v.tage.get(a),s=await I();if(!n){t.innerHTML=`<h1 class="view-title">Tag nicht gefunden</h1><p>Für ${a} liegen keine Daten vor.</p><a href="#/" class="btn">← Dashboard</a>`;return}const i=P(a),l=n.tagestyp==="Stempelzeit"?Ot(n,s):null,c=We(i,s.berufsschulMuster),r=Kt(n.zeitSpannen);t.innerHTML=`
    <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 8px;">
    ...
    <div class="grid cols-3" style="margin-bottom: 24px;">
      ...
      <div class="card">
        <h3>Differenz</h3>
        <div class="tabular" style="font-size: 28px; font-weight: 650; color: ${n.diff>.05?"var(--success)":n.diff<-.05?"var(--danger)":"var(--fg)"};">${n.diff!=null?C(n.diff,{signed:!0}):"—"}</div>
      </div>
    </div>
  ...
  `;const s=()=>n==="alles"?t:t.slice(-n);requestAnimationFrame(()=>{On(e.querySelector("#c-saldo"),a),qn(e.querySelector("#c-wochen"),t),ht(e.querySelector("#c-startend"),s()),Nn(e.querySelector("#c-wochentag"),t),jn(e.querySelector("#c-pausen"),t),Rn(e.querySelector("#c-donut"),t),Hn(e.querySelector("#c-alle-tage"),t.slice(-30))}),(i=e.querySelector("#startend-zeitraum"))==null||i.addEventListener("click",l=>{const c=l.target.closest("button[data-key]");if(!c||c.disabled)return;const r=c.getAttribute("data-key"),o=r==="alles"?"alles":parseInt(r,10);o!==n&&(n=o,e.querySelectorAll("#startend-zeitraum button").forEach(d=>d.classList.toggle("active",d===c)),ht(e.querySelector("#c-startend"),s()))})}async function qe({params:e,root:t}){var ge,re;const[a,n,s,i]=await Promise.all([V(),ie(),I(),ce()]);if(!a.length){t.innerHTML=`
      ...
      <a href="#/daten" class="btn primary">Zum Upload</a>`;return}const l=[...new Set(a.map(M=>M.monatKey))].sort(),c=e.yyyymm,r=l.includes(c)?c:l[l.length-1],o=a.filter(M=>M.monatKey===r),d=[...new Set(o.map(M=>M.wocheKey))].sort(),u=e.yyyykw,g=d.includes(u)?u:d[0],h=n.find(M=>M.monatKey===r),f=((ge=h==null?void 0:h.glz)==null?void 0:ge.saldoGesamt)??null,p=((re=h==null?void 0:h.glz)==null?void 0:re.saldoVorperiode)??null,b=f!=null&&p!=null?+(f-p).toFixed(2):null,m=l.indexOf(r),$=m>0?l[m-1]:null,k=m<l.length-1?l[m+1]:null,y=Zs(r),x=Be(a,n).filter(M=>M.wocheKey===g),K=x.reduce((M,j)=>M+de(j),0),T=x.reduce((M,j)=>M+Me(j),0),H=+(K-T).toFixed(2),F=M=>M>.05?"var(--success)":M<-.05?"var(--danger)":"var(--fg)",G=i!=null&&i.stichtagAuswertung?new Date(i.stichtagAuswertung):new Date,te=Se(a,s,G),ue=te.filter(M=>x.some(j=>j.datum===M.datum)),Ie=te.filter(M=>o.some(j=>j.datum===M.datum)).length,Oe=d.map(M=>{var lt;const he=(lt=o.filter(He=>He.wocheKey===M).sort((He,ra)=>He.datum.localeCompare(ra.datum))[0])==null?void 0:lt.datum,ia=he?$e(P(he)):"";return{wk:M,kw:M.split("-")[1],range:ia}});t.innerHTML=`
    <h1 class="view-title">Kalender</h1>
      ...
      <div class="kalender-monat-saldo">
        ${f!=null?`
          <div class="kalender-monat-saldo-item">
            ...
            <span class="tabular kalender-monat-saldo-wert" style="color: ${F(f)};">${C(f,{signed:!0})}</span>
          </div>
        `:""}
        ${b!=null?`
          <div class="kalender-monat-saldo-item">
            <span class="kalender-monat-saldo-label">Bewegung im Monat</span>
            <span class="tabular kalender-monat-saldo-wert" style="color: ${F(b)};">${C(b,{signed:!0})}</span>
          </div>
        `:""}
      </div>
    </div>
    ...
    <div class="kalender-wochen-tabs" role="tablist" aria-label="Wochen des Monats">
      ${Oe.map(({wk:M,kw:j,range:he})=>`
        <a href="#/kalender/${r}/${M}"
           class="kalender-wochen-tab ${M===g?"active":""}"
           role="tab" aria-selected="${M===g}"
           title="${w(he)}">
          <span class="kalender-wochen-tab-kw">KW ${w(j)}</span>
    ...
    `:`
      ...
      <div class="grid cols-3" style="margin-bottom: 24px;">
        ...
        <div class="card">
          <h3>Soll-Stunden</h3>
          <div class="tabular" style="font-size: 28px; font-weight: 650;">${T.toFixed(2).replace(".",",")}h</div>
        </div>
        <div class="card">
          <h3>Differenz</h3>
          <div class="tabular" style="font-size: 28px; font-weight: 650; color: ${F(H)};">${C(H,{signed:!0})}</div>
        </div>
      </div>
  ...
  `,x.length&&(requestAnimationFrame(()=>{const M=t.querySelector("#chart-kalender-woche");M&&Ve(M,x)}),st(t.querySelector("#tage-liste-kalender")))}function qs(e){return`
    ...
    </div>`}function Zs(e){const[t,a]=e.split("-"),n=new Date(parseInt(t,10),parseInt(a,10)-1,1);return A(n,"MMMM yyyy",{locale:O})}const Ye="Fahrgelderstattung",Gs="C4",ea="C5",ta="F4",aa="F5",oe=10,Mt=19;function zt(e,t){return e.filter(a=>a.monatKey===t).filter(a=>a.tagestyp==="Berufsschule").filter(a=>(a.ist??0)>0).sort((a,n)=>a.datum.localeCompare(n.datum))}async function Us(e){const t=(await tt(async()=>{const{default:p}=await import("./exceljs.min-BhfpbegZ.js").then(b=>b.e);return{default:p}},__vite__mapDeps([0,1]))).default,a=new t.Workbook;await a.xlsx.load(e);const n=a.getWorksheet(Ye)??a.worksheets[0];if(!n)return{ok:!1,fehler:"Excel hat keine Arbeitsblätter."};const s=p=>{const b=n.getCell(p).value;return b==null?"":typeof b=="object"&&b.text?String(b.text):typeof b=="object"&&b.richText?b.richText.map(m=>m.text).join(""):String(b)},i=s(ea).trim(),l=s(ta).trim(),c=s(aa).trim(),r=(l.match(/\d+/)||[""])[0],o=(c.match(/\d+/)||[""])[0],d=s(`C${oe}`).trim(),u=s(`E${oe}`).trim(),g=n.getCell(`G${oe}`).value,h=typeof g=="number"?g:parseFloat(String(g).replace(",","."))||0;let f=null;try{const p=new Set(n.getImages().map(m=>parseInt(m.imageId,10))),b=(a.media||[]).map((m,$)=>({media:m,index:$})).filter(({media:m,index:$})=>(m==null?void 0:m.type)==="image"&&!p.has($)&&m.buffer);if(b.length>0){b.sort((y,S)=>(S.media.buffer.length||0)-(y.media.buffer.length||0));const m=b[0].media,$=m.buffer,k=$ instanceof ArrayBuffer?$:$.buffer?$.buffer.slice($.byteOffset||0,($.byteOffset||0)+$.byteLength):null;if(k){const y=(m.extension||"").toLowerCase();f={bytes:k,extension:y==="jpg"?"jpeg":y||"png",quelle:"auto-aus-vorlage"}}}}catch(p){console.warn("[fahrtgeld] Unterschrift-Auto-Extraktion fehlgeschlagen:",p)}return{ok:!0,konstanten:{name:i,persNr:r,kst:o,vonHaltestelle:d,nachHaltestelle:u,betragProTag:h},unterschriftAuto:f}}async function Js({templateBytes:e,monatKey:t,bsTage:a,konstanten:n,unterschriftBytes:s,unterschriftExtension:i}){const l=(await tt(async()=>{const{default:x}=await import("./exceljs.min-BhfpbegZ.js").then(K=>K.e);return{default:x}},__vite__mapDeps([0,1]))).default,c=new l.Workbook;await c.xlsx.load(e);const r=c.getWorksheet(Ye)??c.worksheets[0];if(!r)throw new Error(`Sheet "${Ye}" nicht in der Vorlage gefunden.`);const[o,d]=t.split("-").map(x=>parseInt(x,10)),u=new Date(o,d-1,1,12,0,0),g=A(u,"MMMM yy",{locale:O}),h=r.getCell(Gs);h.value=g,h.numFmt="@",r.getCell(ea).value=n.name,r.getCell(ta).value=`Pers.-Nr.: ${n.persNr}`,r.getCell(aa).value=`KST: ${n.kst}`;for(let x=oe;x<=Mt;x++)r.getCell(`A${x}`).value=null,r.getCell(`C${x}`).value=null,r.getCell(`E${x}`).value=null,r.getCell(`G${x}`).value=null;const f=Mt-oe+1,p=a.slice(0,f);for(let x=0;x<p.length;x++){const K=p[x],T=oe+x;r.getCell(`A${T}`).value=P(K.datum),r.getCell(`C${T}`).value=n.vonHaltestelle,r.getCell(`E${T}`).value=n.nachHaltestelle,r.getCell(`G${T}`).value=n.betragProTag}const m=A(new Date,"dd.MM.yyyy"),$=r.getCell("A21");if($.value=m,$.alignment={vertical:"bottom",horizontal:"center",wrapText:!1},s&&i){const x=c.addImage({buffer:s,extension:i});r.addImage(x,{tl:{col:.15,row:20.02},ext:{width:180,height:50},editAs:"oneCell"})}const k=await c.xlsx.writeBuffer(),y=new Blob([k],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}),S=Ys(n.name,u);return{blob:y,dateiname:S,anzahlTage:p.length,ueberzaehlig:a.length-p.length}}function Ys(e,t){const a=A(t,"MMMM",{locale:O}),n=A(t,"yy");let s=e;const i=/^(.+?),\s*(.+)$/.exec(e);return i&&(s=`${i[2].trim()} ${i[1].trim()}`),`Fahrtgeld ${s} ${a} ${n}.xlsx`}function Xs(e){const[t,a]=e.split("-").map(n=>parseInt(n,10));return A(new Date(t,a-1,1),"MMMM yyyy",{locale:O})}function Qs(e){const t=new Set;for(const a of e)a.tagestyp==="Berufsschule"&&(a.ist??0)>0&&t.add(a.monatKey);return[...t].sort().reverse()}function ei(e,t,a,n){var g,h;const s=t==null?void 0:t.berufsschulMuster;if(!s||!((g=s.wochentageWoechentlich)!=null&&g.length)&&!((h=s.wochentage14Taegig)!=null&&h.length))return{ausstehend:[],imMonatGesamt:a.length,vorhandenAnzahl:a.length};const[i,l]=e.split("-").map(f=>parseInt(f,10)),c=new Date(i,l-1,1),r=new Date(i,l,0),o=new Set(a.map(f=>f.datum)),d=[];let u=new Date(c);for(;u<=r;){if(!et(u)&&We(u,s)){const f=A(u,"yyyy-MM-dd");u>n&&!o.has(f)&&d.push(f)}u=Qe(u,1)}return{ausstehend:d,imMonatGesamt:a.length+d.length,vorhandenAnzahl:a.length}}function ti(e,t){const a=URL.createObjectURL(e),n=document.createElement("a");n.href=a,n.download=t,document.body.appendChild(n),n.click(),document.body.removeChild(n),setTimeout(()=>URL.revokeObjectURL(a),1e3)}async function ke({root:e}){var d,u,g,h,f,p,b,m,$;const t=await we();if(!t){ai(e);return}const[a,n,s]=await Promise.all([V(),I(),ce()]),i=s!=null&&s.stichtagAuswertung?new Date(s.stichtagAuswertung):new Date,l=Qs(a),c=new Date,r=`${c.getFullYear()}-${String(c.getMonth()+1).padStart(2,"0")}`;l.includes(r)||l.unshift(r);const o=l;e.innerHTML=`
    ...
    `:`
      <div class="card">
        ...
        <div class="fahrtgeld-monate">
          ${o.map(k=>{const y=zt(a,k),S=+(y.length*(t.konstanten.betragProTag??0)).toFixed(2),x=y.length>10,K=ei(k,n,y,i),T=K.ausstehend.length,H=+((y.length+T)*(t.konstanten.betragProTag??0)).toFixed(2),F=T>0?K.ausstehend.map(te=>L(P(te))).join(", "):"",G=`data-monat="${w(k)}" data-offen="${T}" data-offen-liste="${w(F)}"`;return`
              <div class="fahrtgeld-monat-row${T>0?" has-pending":""}">
                <div class="fahrtgeld-monat-titel">
                  ...
                  <div class="hint">
                    ...
                    ${T>0?`<br><span class="fahrtgeld-pending">⏳ Laut deinem Schul-Rhythmus kommen noch ${T} BS-Tag${T===1?"":"e"}: ${w(F)}. Mit allen wären's ${y.length+T} Tage = ${H.toFixed(2).replace(".",",")} €.</span>`:""}
                    ${x?`<br><span style="color: var(--warn); font-weight: 600;">· ${y.length-10} überzählig (max 10/Monat)</span>`:""}
                  </div>
                </div>
                <button class="btn primary fahrtgeld-generieren" ${G} ${y.length===0?"disabled":""}>
  ...
  `,(h=e.querySelector("#btn-template-replace"))==null||h.addEventListener("click",()=>{e.querySelector("#fahrtgeld-replace-input").click()}),(f=e.querySelector("#fahrtgeld-replace-input"))==null||f.addEventListener("change",async k=>{var S;const y=(S=k.target.files)==null?void 0:S[0];y&&await na(y,e)}),(p=e.querySelector("#btn-template-clear"))==null||p.addEventListener("click",async()=>{confirm("Vorlage wirklich löschen? Beim nächsten Öffnen wirst du gebeten, eine neue hochzuladen.")&&(await qa(),E("Vorlage gelöscht."),ke({root:e}))}),(b=e.querySelector("#btn-signature-upload"))==null||b.addEventListener("click",()=>{e.querySelector("#fahrtgeld-signature-input").click()}),(m=e.querySelector("#fahrtgeld-signature-input"))==null||m.addEventListener("change",async k=>{var S;const y=(S=k.target.files)==null?void 0:S[0];y&&await ni(y,e)}),($=e.querySelector("#btn-signature-clear"))==null||$.addEventListener("click",async()=>{if(!confirm("Unterschrift wirklich entfernen?"))return;const k=await we();k&&(await at({...k,unterschrift:null}),E("Unterschrift entfernt."),ke({root:e}))}),e.querySelectorAll(".fahrtgeld-generieren").forEach(k=>{k.addEventListener("click",async()=>{var T,H;const y=k.getAttribute("data-monat"),S=parseInt(k.getAttribute("data-offen")||"0",10),x=k.getAttribute("data-offen-liste")||"";if(S>0&&!confirm(`Achtung: Laut deinem Schul-Rhythmus kommen in diesem Monat noch ${S} BS-Tag${S===1?"":"e"}:

${x}

Diese Tage sind noch nicht im Zeitnachweis-PDF und werden in der Excel fehlen.

Trotzdem jetzt für die bisher erfassten Tage generieren?`))return;k.disabled=!0;const K=k.textContent;k.textContent="Erstelle …";try{const F=await we(),G=await V(),te=zt(G,y);if(!te.length){R("Für diesen Monat sind keine BS-Anwesenheits-Tage erkannt.",{type:"warn"});return}const ue=(T=F.unterschrift)==null?void 0:T.bytes,Ie=(H=F.unterschrift)==null?void 0:H.extension,{blob:Oe,dateiname:ge,ueberzaehlig:re}=await Js({templateBytes:F.bytes,monatKey:y,bsTage:te,konstanten:F.konstanten,unterschriftBytes:ue,unterschriftExtension:Ie});ti(Oe,ge),E(`Excel erstellt: ${ge}${re>0?` (${re} weiterer Tag${re===1?"":"e"} passte/n nicht in die Vorlage)`:""}`)}catch(F){console.error("Fahrtgeld-Generierung fehlgeschlagen:",F),R(`Fehler: ${F.message||F}`,{type:"error",duration:8e3})}finally{k.disabled=!1,k.textContent=K}})})}function ai(e){var t,a;e.innerHTML=`
    <h1 class="view-title">Fahrtgeld</h1>
  ...
  `,(t=e.querySelector("#btn-template-upload"))==null||t.addEventListener("click",()=>{e.querySelector("#fahrtgeld-upload-input").click()}),(a=e.querySelector("#fahrtgeld-upload-input"))==null||a.addEventListener("change",async n=>{var i;const s=(i=n.target.files)==null?void 0:i[0];s&&await na(s,e)})}async function ni(e,t){try{if(!["image/png","image/jpeg"].includes(e.type)&&!/\.(png|jpe?g)$/i.test(e.name)){R("Bitte ein PNG oder JPG hochladen.",{type:"warn"});return}const n=await e.arrayBuffer(),s=/\.png$/i.test(e.name)||e.type==="image/png"?"png":"jpeg",i=await new Promise((c,r)=>{const o=new FileReader;o.onload=()=>c(o.result),o.onerror=r,o.readAsDataURL(e)}),l=await we();if(!l){R("Bitte zuerst eine Excel-Vorlage hochladen.",{type:"warn"});return}await at({...l,unterschrift:{bytes:n,extension:s,dataUrl:i,hochgeladenAm:new Date().toISOString()}}),E("Unterschrift gespeichert."),ke({root:t})}catch(a){console.error("Unterschrift-Upload fehlgeschlagen:",a),R(`Fehler beim Verarbeiten: ${a.message||a}`,{type:"error",duration:8e3})}}async function na(e,t){try{const a=await e.arrayBuffer(),n=await Us(a);if(!n.ok){R(`Vorlage nicht erkannt: ${n.fehler}`,{type:"error",duration:8e3});return}const s=await we();let i=s==null?void 0:s.unterschrift;if(!i&&n.unterschriftAuto){const l=si(n.unterschriftAuto.bytes,n.unterschriftAuto.extension);i={bytes:n.unterschriftAuto.bytes,extension:n.unterschriftAuto.extension,dataUrl:l,quelle:"auto-aus-vorlage",hochgeladenAm:new Date().toISOString()}}await at({bytes:a,konstanten:n.konstanten,unterschrift:i,hochgeladenAm:new Date().toISOString(),dateiname:e.name}),n.unterschriftAuto&&!(s!=null&&s.unterschrift)?E(`Vorlage gespeichert: ${n.konstanten.name||e.name} (Unterschrift aus Vorlage übernommen)`):E(`Vorlage gespeichert: ${n.konstanten.name||e.name}`),ke({root:t})}catch(a){console.error("Vorlage-Upload fehlgeschlagen:",a),R(`Fehler beim Verarbeiten: ${a.message||a}`,{type:"error",duration:8e3})}}function si(e,t){const a=new Uint8Array(e);let n="";for(let l=0;l<a.length;l++)n+=String.fromCharCode(a[l]);const s=btoa(n);return`data:${t==="png"?"image/png":"image/jpeg"};base64,${s}`}async function ii({root:e}){const t=await V();if(!t.length){e.innerHTML='<h1 class="view-title">Berichtsheft</h1><p class="view-subtitle">Keine Daten.</p><a href="#/daten" class="btn primary">Zum Upload</a>';return}const a=[...new Set(t.map(i=>i.monatKey))].sort().reverse();e.innerHTML=`
...
`);navigator.clipboard.writeText(l).then(()=>E(`${i.length} Zeilen kopiert.`))});function s(){const i=n.value,c=(i?t.filter(r=>r.monatKey===i):t).filter(r=>r.ist!=null&&r.ist>0&&r.tagestyp!=="Wochenende");e.querySelector("#liste").innerHTML=ri(c),e.querySelectorAll(".copy-val").forEach(r=>{r.addEventListener("click",async()=>{const o=r.getAttribute("data-val");await navigator.clipboard.writeText(o),r.classList.add("ok");const d=r.textContent;r.textContent="✓ kopiert",setTimeout(()=>{r.classList.remove("ok"),r.textContent=d},1200)})})}s()}function ri(e){return e.length?`
    <table class="table">
      ...
      <tbody>
        ${e.map(t=>{const a=Le(t.ist),n=`${L(P(t.datum))}	${a}`;return`
            <tr data-copy="${Dt(n)}">
              <td class="tabular">${L(P(t.datum))}</td>
...
${n}`;await navigator.clipboard.writeText(s),E("Mail-Text kopiert.")})})}function _(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function Xe(e){return _(e)}async function pi({root:e}){const[t,a]=await Promise.all([I(),nt()]),n=Object.fromEntries(a.map(s=>[s.key,s]));e.innerHTML=`
  ...
  `}function Te(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function vi(e){return Te(e)}async function At(){document.documentElement.setAttribute("data-css-ready",""),La();const e=document.getElementById("content");await Oa()&&setTimeout(()=>{R('Parser wurde aktualisiert. Bitte lade deinen Zeitnachweis unter "Daten" neu hoch.',{type:"info",duration:12e3})},500),await Ia(),await _a(),W("/",pt),W("/dashboard",pt),W("/analytics",Rs),W("/kalender",qe),W("/kalender/:yyyymm",qe),W("/kalender/:yyyymm/:yyyykw",qe),W("/monat",Fs),W("/monat/:yyyymm",Ks),W("/woche",_s),W("/woche/:yyyykw",Is),W("/tag/:datum",Hs),W("/berichtsheft",ii),W("/mails",di),W("/fahrtgeld",ke),W("/profil",Ls),W("/daten",Ms),W("/einstellungen",pi),Ka(e),location.hash||Fa("/");try{const t=await v.tage.count();bs(t>0)&&setTimeout(()=>Jt(),700)}catch(t){console.warn("[boot] Tour-Auto-Start-Check fehlgeschlagen:",t)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",At):At();
